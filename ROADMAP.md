# Roadmap

Noturcode publishes capabilities only after an end-to-end test proves them.

## Stable core

- Claude Code and Codex lifecycle status in the notch
- local transcript rendering and semantic tool batches
- exact iTerm2 pane return and prompt delivery
- completion/attention notifications
- local files, Markdown, code, images, models, and token totals
- Claude parent/subagent conversation navigation

## Next

- signed and notarized releases with reproducible checksums
- integration Doctor with preview, repair, restore, and ownership manifest
- reliable tmux and SSH session identity/forwarding
- exact navigation adapters for Terminal, Ghostty, Warp, WezTerm, and kitty
- Codex subagent/team parity as upstream events become available
- Gemini and OpenCode transcript adapters
- performance budgets for animation, transcript polling, and multiple displays

## Explicitly not promised yet

- remote control through a hosted service
- mobile companion applications
- automatic approval of agent permissions
- identical features across every detected harness or terminal

Open issues should describe the exact harness, terminal, local/SSH topology, and expected session identity. Reliability beats a large compatibility logo wall.
